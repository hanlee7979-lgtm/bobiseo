...
> **업데이트 (M6 일부 선행, 2026-08-08)**: 소비자 홈(`/[locale]`)이 로그인 게이트가 아니라
> 비로그인 상태에서도 바로 LOCAL HERO 피드를 보여주는 것으로 바뀌었다. "Google로 계속하기"는
> 화면 우측 상단(`TopBar` → `AuthHeaderControls`)에 위치하고, 클릭하면 **풀페이지 이동이 아니라
> 모달**로 로그인·역할 선택이 뜬다. 아래 §1 다이어그램과 `/login`·`/onboarding` 풀페이지는
> `proxy.ts`가 보호 라우트(`/crm`, `/home` 등) 접근을 막을 때의 **폴백 경로**로만 남아 있다.

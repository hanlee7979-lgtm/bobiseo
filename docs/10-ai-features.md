...
export interface AiAdapter {
  ...
  redactAnonymous(input: { text: string }): Promise<AiJob<{
    text: string
...
const PATTERNS: { type: Redaction['type']; re: RegExp; replace: string }[] = [
  ...
  { type: 'address', re: /[가-힣]+(시|군|구)\s?[가-힣0-9]+(로|길|동|읍|면)\s?\d+(-\d+)?/g, replace: '○○○' },
  { type: 'store',   re: /[가-힣A-Za-z0-9]{2,}\s?(식당|카페|베이커리|미용실|정육점|마트|치킨|분식)/g, replace: '○○○' },
]
```
